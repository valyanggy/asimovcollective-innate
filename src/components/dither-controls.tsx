...
type DitherGroup = { id: string; title: string; description: string; controls: { key: DitherSliderKey; label: string; min: number; max: number; step: number; unit?: string }[] };
const groups: DitherGroup[] = [
  ...
  { id: 'domain', title: 'Domain connection', description: 'Increase adhesion and reach to bring nearby domains together.', controls: [
    ...
    {key:'reach',label:'Influence reach',min:.1,max:6,step:.05},
  ]},
  { id: 'tone', title: 'Tone', description: 'Shadow area shapes the whole field. Center shadow tightens only the main image; Center rotation tilts that island.', controls: [
    {key:'shadow',label:'Shadow area',min:.1,max:20,step:.05},
    {key:'centerShadow',label:'Center shadow',min:.1,max:20,step:.05},
    {key:'centerRotation',label:'Center rotation',min:0,max:360,step:1,unit:'°'},
    {key:'contrast',label:'Contrast',min:.5,max:3,step:.05},
    {key:'softness',label:'Edge softness',min:.04,max:1.65,step:.01},
  ]},
  { id: 'shading', title: 'Shading', description: 'Light reveals the slopes and depths of the shared density field.', controls: [
    {key:'relief',label:'Surface relief',min:0,max:2,step:.05},
    {key:'lightAngle',label:'Light direction',min:0,max:360,step:5,unit:'°'},
  ]},
  { id: 'cells', title: 'Cells', description: 'Size changes each mark; spacing changes the distance between marks. Corner radius fillets free outer corners.', controls: [
...
function sliderGroups(
  ...
  return items.map(group => <fieldset key={group.id} className={styles.group}>
    <legend>{group.title}</legend><p>{group.description}</p>
    {group.controls.map(control => {
      const id = `dither-${control.key}`;
...
export function DitherControls({value,onChange,led,field,onReplay,defaults,gridOnly,rimBlocks}:{value:DitherFieldSettings;onChange:(settings:DitherFieldSettings)=>void;led?:boolean;field?:boolean;onReplay?:()=>void;defaults?:DitherFieldSettings;gridOnly?:boolean;rimBlocks?:boolean}) {
  ...
  return <section className={`${styles.panel} ${gridOnly ? styles.dock : ""}`} aria-label="Field adjustments">
    ...
    {showField && sliderGroups(groups.filter(group => group.id !== 'rim' || rimBlocks), value, onChange)}
